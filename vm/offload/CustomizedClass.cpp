...
ClassObject* customResolveClass(const ClassObject* referrer, u4 classIdx,
...
bool customLinkClass(ClassObject* clazz)
{
    ...
    if (clazz->status == CLASS_IDX) {
        ...
        if (superclassIdx != kDexNoIndex) {
            ClassObject* super = customResolveClass(clazz, superclassIdx, false);
            if (super == NULL) {
                if (gDvm.optimizing) {
                    /* happens with "external" libs */
        ...
        if (clazz->interfaceCount > 0) {
            ...
            for (i = 0; i < clazz->interfaceCount; i++) {
                ...
                clazz->interfaces[i] =
                    customResolveClass(clazz, interfaceIdxArray[i], false);
...
static bool computeFieldOffsets(ClassObject* clazz)
{
     ...
     */
     bool isclass = !dvmIsTheClassClass(clazz);
     bool assertResult = !dvmIsTheClassClass(clazz) || (size_t)fieldOffset <
        OFFSETOF_MEMBER(ClassObject, instanceData) + sizeof(clazz->instanceData);
     if(strncmp(clazz->descriptor, "Ljava/lang/Class;", 17) == 0) {
     ALOGE("clazz is: %s, isclass: %d, offset is: %d, the other is: %d, the otter is: %d, assert result is: %d", clazz->descriptor, isclass, fieldOffset, OFFSETOF_MEMBER(ClassObject, instanceData), sizeof(clazz->instanceData), assertResult);
     }
