...
void showBranchStats() {
    std::map<int, int> bdepthnum;
    for (std::map<Method*, MethodAccInfo*>::iterator it = methodResMap.begin(); it != methodResMap.end(); ++it) {
        int depth = it->second->bdepth;
        if(bdepthnum.find(depth) == bdepthnum.end()) {
            bdepthnum[depth] = 0;
        }
        bdepthnum[depth] = bdepthnum[depth] + 1;
    }
    
    for (std::map<int, int>::iterator it = bdepthnum.begin(); it != bdepthnum.end(); ++it) {
        ALOGE("depth %d have method num: %d", it->first, it->second);
    }
}
    ...
    if (dexFindCatchHandler(&iterator, pCode, relPc)) {
        for (;;) {
            ...
            if(!isCycle) {
                ...
                if(newDepth > methodAccInfo->bdepth) {
                    methodAccInfo->bdepth = newDepth;
                }
...
void retrieveMethodInfo(std::vector<MethodAccResult*>* methodAccVec, const char* fileName) {
    ...
    while(std::getline(srcfile, line)) {
        ...
        methodAccVec->push_back(methodAccResult);
