...
void mergeObjectAccTrackInfo(std::vector<ObjectAccInfo*>* dstAccInfoList, ObjectAccInfo* srcAccInfo, std::map<ObjectAccInfo*, std::vector<ObjectAccInfo*> >* addrMap, std::vector<ObjectAccInfo*>* changeCache);
...
void mergeObjectTrack(ObjectAccInfo* dstAccInfo, ObjectAccInfo* srcAccInfo, std::map<ObjectAccInfo*, std::vector<ObjectAccInfo*> >* addrMap, std::vector<ObjectAccInfo*>* changeCache) {
    //pushSingle(changeCache, dstAccInfo);
    changeCache->push_back(dstAccInfo);
...
void mergeGlobalClazzTrack(MethodAccInfo* methodAccInfo, MethodAccInfo* subAccInfo, std::map<ObjectAccInfo*, std::vector<ObjectAccInfo*> >* addrMap, std::vector<ObjectAccInfo*>* changeCache) {
    for(unsigned int i = 0; i < subAccInfo->globalClazz->size(); i++) {
        unsigned int j;
        for(j = 0; j < methodAccInfo->globalClazz->size(); j++) {
            if(subAccInfo->globalClazz->at(i)->clazz == methodAccInfo->globalClazz->at(j)->clazz) {
...
void applyMergeToTrack(std::vector<ObjectAccInfo*>* changeCache) {
    for(unsigned int i = 0; i < changeCache->size(); i++) {
        ObjectAccInfo* objAccInfo = changeCache->at(i);
...
void mergeMethodArgs(u2 vsrc1, u2 vdst, std::map<u2, std::vector<ObjectAccInfo*> >* interestRegObjMap, MethodAccInfo* methodAccInfo, MethodAccInfo* subAccInfo) {
    ...
    std::vector<ObjectAccInfo*>* changeCache = new std::vector<ObjectAccInfo*>();
    switch (count) {
    case 5:
        reg = vsrc1 & 0x0f;
        if(interestRegObjMap->find(reg) != interestRegObjMap->end()) {
