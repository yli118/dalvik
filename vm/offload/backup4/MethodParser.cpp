...
void mergeObjectAccFieldInfo(std::vector<ObjectAccInfo*>* accInfoList1, ObjectAccInfo* accInfo2, std::map<ObjectAccInfo*, std::vector<ObjectAccInfo*> >* addrMap) {
    (*addrMap)[accInfo2] = *accInfoList1;
    if(accInfo2->allFlag) {
        for(unsigned int i = 0; i < accInfoList1->size(); i++) {
            accInfoList1->at(i)->allFlag = true;
        }
        return;
    }
    for(unsigned int i = 0; i < accInfoList1->size(); i++) {
        if(accInfoList1->at(i)->fieldSet.size() < accInfo2->fieldSet.size()) {
            accInfoList1->at(i)->fieldSet.resize(accInfo2->fieldSet.size());
            accInfoList1->at(i)->trackSet.resize(accInfo2->trackSet.size());
        }
    }
    for(unsigned int i = 0; i < accInfo2->fieldSet.size(); i++) {
        if(accInfo2->fieldSet[i] == NULL) {
            continue
        }
        std::vector<ObjectAccInfo*> fieldList;
        for(unsigned int j = 0; j < accInfoList1->size(); j++) {
            if(accInfoList1->at(j)->fieldSet[i] == NULL) {
                accInfoList1->at(j)->fieldSet[i] = new ObjectAccInfo();
                accInfoList1->at(j)->fieldSet[i]->belonging = accInfoList1->at(j);
            }
            fieldList.push_back(accInfoList1->at(j)->fieldSet[i]);
        }
        mergeObjectAccInfo(&fieldList, accInfo2->fieldSet[i], addrMap);
    }
    ...
    if(accInfo2->isReturn) {
        if(returnInterests == NULL) {
            returnInterests = new std::vector<ObjectAccInfo*>();
        }
        bool inReturn = false;
        for(unsigned int i = 0; i < accInfoList1->size(); i++) {
            for(unsigned int j = 0; j < returnInterests->size(); j++) {
                if(accInfoList1->at(i) == returnInterests->at(j)) {
                    inReturn = true;
                    break;
                }
                if(!inReturn) {
                    returnInterests->push_back(accInfoList1->at(i));
                }
            }
        }
    }
...
void mergeObjectAccTrackInfo(std::vector<ObjectAccInfo*>* accInfoList1, ObjectAccInfo* accInfo2, std::map<ObjectAccInfo*, std::vector<ObjectAccInfo*> >* addrMap) {
    for(unsigned int i = 0; i < accInfo2->trackSet.size(); i++) {
        if(accInfo2->trackSet[i] == NULL || accInfo2->trackSet[i]->size() == 0) {
            continue
        }
        for(unsigned int j = 0; j < accInfo2->trackSet[i]->size(); j++) {
            if(accInfo2->fieldSet[i] == accInfo2->trackSet[i]->at(j)) {
                for(unsigned int k = 0; k < accInfoList1->size(); k++) {
                    bool inTrack = false;
                    for(unsigned int l = 0; l < accInfoList1->at(k)->trackSet[i]->size(); l++) {
                        if(accInfoList1->at(k)->trackSet[i]->at(l) == accInfoList1->at(k)->fieldSet[i]) {
                            inTrack = true;
                            break;
                        }
                    }
                    if(!inTrack) {
                        accInfoList1->at(k)->trackSet[i]->push_back(accInfoList1->at(k)->fieldSet[i]);
                    }
                }
            } else {
                for(unsigned int k = 0; k < accInfoList1->size(); k++) {
                    for(unsigned int l = 0; l < (*addrMap)[accInfo2->trackSet[i]->at(j)].size(); l++) {
                        bool inTrack = false;
                        for(unsigned int m = 0; m < accInfoList1->at(k)->trackSet[i]->size(); m++) {
                            if((*addrMap)[accInfo2->trackSet[i]->at(j)].at(l) == accInfoList1->at(k)->trackSet[i]->at(m)) [
                                inTrack = true;
                                break;
                            }
                        }
                        if(!inTrack) {
                            accInfoList1->at(k)->trackSet[i]->push_back((*addrMap)[accInfo2->trackSet[i]->at(j)].at(l));
                        }                       
                    }
                }
            }
        }
        mergeObjectAccInfo(accInfo1->fieldSet[i], accInfo2->fieldSet[i], addrMap);
    }
}
