...
void unionOjbectFieldInfo(ObjectAccInfo* dstObjAccInfo, ObjectAccInfo* srcObjAccInfo, std::map<ObjectAccInfo*, ObjectAccInfo*>* addrMap) {
    (*addrMap)[srcObjAccInfo] = dstObjAccInfo;
    if(srcObjAccInfo->allFlag || dstObjAccInfo->allFlag) {
    ...
    for(unsigned int i = 0; i < srcObjAccInfo->fieldSet.size(); i++) {
        ...
        unionOjbectFieldInfo(dstObjAccInfo->fieldSet[i], srcObjAccInfo->fieldSet[i], addrMap);
    }
}

void unionOjbectTrackInfo(ObjectAccInfo* dstObjAccInfo, ObjectAccInfo* srcObjAccInfo, std::map<ObjectAccInfo*, ObjectAccInfo*>* addrMap) {
    for(unsigned int i = 0; i < srcObjAccInfo->trackSet.size(); i++) {
        if(dstObjAccInfo->fieldSet[i] != NULL && srcObjAccInfo->fieldSet[i] != NULL) {
            unionOjbectTrackInfo(dstObjAccInfo->fieldSet[i], srcObjAccInfo->fieldSet[i], addrMap);
        }
...
void unionClazzFieldInfo(MethodAccInfo* dstAccInfo, MethodAccInfo* srcAccInfo, std::map<ObjectAccInfo*, ObjectAccInfo*>* addrMap) {
    for(unsigned int i = 0; i < srcAccInfo->globalClazz->size(); i++) {
        ...
        unionOjbectFieldInfo(dstAccInfo->globalClazz->at(j), srcAccInfo->globalClazz->at(i), addrMap);
    }
}
...
void unionClazzTrackInfo(MethodAccInfo* dstAccInfo, MethodAccInfo* srcAccInfo, std::map<ObjectAccInfo*, ObjectAccInfo*>* addrMap) {
    for(unsigned int i = 0; i < srcAccInfo->globalClazz->size(); i++) {
        ...
        unionOjbectTrackInfo(dstAccInfo->globalClazz->at(j), srcAccInfo->globalClazz->at(i), addrMap)
    }
}
...
void unionMethodAccInfo(MethodAccInfo* dstAccInfo, MethodAccInfo* srcAccInfo) {
    ...
    for(unsigned int i = 0; i < srcAccInfo->args->size(); i++) {
        unionOjbectFieldInfo(dstAccInfo->args->at(i), srcAccInfo->args->at(i), &addrMap);
    }
    ...
    for(unsigned int i = 0; i < srcAccInfo->args->size(); i++) {
        unionOjbectTrackInfo(dstAccInfo->args->at(i), srcAccInfo->args->at(i), &addrMap);
    }
...
void mergeObjectAccFieldInfo(std::vector<ObjectAccInfo*>* dstAccInfoList, ObjectAccInfo* srcAccInfo, std::map<ObjectAccInfo*, std::vector<ObjectAccInfo*> >* addrMap) {
    ...
    for(unsigned int i = 0; i < dstAccInfoList->size(); i++) {
        if(dstAccInfoList->at(i)->allFlag) {
            continue;
        }
    ...
    for(unsigned int i = 0; i < srcAccInfo->fieldSet.size(); i++) {
        ...
        for(unsigned int j = 0; j < dstAccInfoList->size(); j++) {
            if(dstAccInfoList->at(j)->allFlag) {
                continue;
            }
...
void mergeObjectTrack(ObjectAccInfo* dstAccInfo, ObjectAccInfo* srcAccInfo, std::map<ObjectAccInfo*, std::vector<ObjectAccInfo*> >* addrMap) {
    for(unsigned int i = 0; i < srcAccInfo->trackSet.size(); i++) {
        if(dstAccInfo->fieldSet[i] != NULL && srcAccInfo->fieldSet[i] != NULL) {
            mergeObjectTrack(dstAccInfo->fieldSet[i], srcAccInfo->fieldSet[i], addrMap);
        }
...
void mergeMethodArgs(u2 vsrc1, u2 vdst, std::map<u2, std::vector<ObjectAccInfo*>* >* interestRegObjMap, MethodAccInfo methodAccInfo, MethodAccInfo* subAccInfo) {
    u4 count = vsrc1 >> 4;
    u2 reg;
    std::vector<ObjectAccInfo*> > addrMap;
    switch (count) {
    case 5:
        reg = vsrc1 & 0x0f;
        if(interestRegObjMap->find(reg) != interestRegObjMap->end()) {
...
bool rangeMethodHasInterest(u2 vsrc1, u2 vdst, std::map<u2, std::vector<ObjectAccInfo*>* >* interestRegObjMap) {
    ...
    for(int i = 0; i < vsrc1; i++) {
        if(interestRegObjMap->find(vdst + i) != interestRegsObjMap->end() && !isVecFlagedAll((*interestRegObjMap)[vdst + i])) {
            hasInterest = true;
            break;
        }
    }
...
void parseInsns(const u2* insns, MethodAccInfo* methodAccInfo, std::vector<Method*>* chain, std::vector<int>* insOffsets, std::map<u2, std::vector<ObjectAccInfo*>* >* interestRegObjMap, int depth, bool* exitMethod) {
    ...
    for( ; ; lastOpcode = opcode, insns += width) {
        ...
        if(opcode == OP_IGET || opcode == OP_IGET_WIDE || opcode == OP_IGET_OBJECT || opcode == OP_IGET_BOOLEAN 
            ...
            if(virtualResMap->find(baseMethod) != NULL) {
                ...
                for(unsigned int idx = 0; idx < subclasses.size(); idx++) {
                    ...
                    if(methodResMap.find(methodToCall) == methodResMap.end()) {
                        ...
                        toCallAccInfo = new MethodAccInfo();
                        toCallAccInfo->method = methodToCall;
                        populateMethodAccInfo(toCallAccInfo);
                ...
                if(virtualResMap->find(baseMethod) == virtualResMap->end()) {
                    virtualResMap.push_back(baseMethod, subAccInfo);
                } else {
                    free(subAccInfo);
                }
            }
            ...
            //ALOGE("methodParser parse virtual %s.%s", baseMethod->clazz->descriptor, baseMethod->name);
            MethodAccInfo* subAccInfo;
            if(interResMap->find(absMethod) != NULL) {
                subAccInfo = interResMap->at(absMethod);
            } else {
                subAccInfo = new MethodAccInfo();
                ...
                if(interResMap->find(baseMethod) == interResMap->end()) {
                    interResMap.push_back(absMethod, subAccInfo);
                } else {
                    free(subAccInfo);
                }
            }
...
void parseMethod(MethodAccInfo* methodAccInfo, std::vector<Method*>* chain) {
    ...
    if(isCycle || dvmIsNativeMethod(method) || dvmIsAbstractMethod(method)) {
        for(unsigned int idx = 0; idx < methodAccInfo->args->size(); i++) {
    ...
    for(int i = 0; i < method->insSize; i++) {
        if(i == 0 && !dvmIsStaticMethod(method)) {
            interestRegObjMap[method->registersSize - method->insSize + i] = new std::vector<ObjectAccInfo*>();
