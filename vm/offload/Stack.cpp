...
void offPushStackLocal(FifoBuffer* fb, Thread* thread) {
  ...
  //ObjectInfo* threadInfo = offIdObjectInfo(auxObjectToId(thread->threadObj));
  //threadInfo->allFlag = true;
  //ObjectInfo* exInfo = offIdObjectInfo(auxObjectToId(thread->exception));
  //exInfo->allFlag = true;
...
*/
  ...
  if(fp == NULL) {
    ...
    if(prevFp == NULL) {
        ...
        stackOffset = (u1*)prevFp - (u1*)fp; //TODO: might be wrong, see if we need to add or subtract another sizeof(StackSaveArea)
    }
  }
  writeU4(fb, stackOffset); // stack offset
  writeU4(fb, 0); // thread->breakFrames breaks as zero
...
//          ALOGE("value of i is: %d, regsize is: %d", i, method->registersSize);
        ...
        for(i = 0; i < method->registersSize; i++) {
          ...
          if(i >= argStart && (bits & 0x01)) {
            ...
            if(obj) offAddTrackedObject(obj);
            rval = auxObjectToId(obj);
            ALOGE("get object id: %d, object name: %s", rval, obj->clazz->descriptor);
          }
...
*/
  ...
  //writeU4(fb, 44); //set as one frame for calculate
  //writeU4(fb, 64); // set as one fframe for matrixTest
  //writeU4(fb, 0); // set as no break frame
