...
void freeMethodAccInfo(MethodAccInfo* methodAccInfo) {
    ...
    if(methodAccInfo->globalClazz != NULL) {
        ...
        free(methodAccInfo->globalClazz);
    }
    if(methodAccInfo->returnObjs != NULL) {
        free(methodAccInfo->returnObjs);
    }
    if(methodAccInfo->curMethodReturns != NULL) {
        free(methodAccInfo->curMethodReturns);
    }
}
...
void unionObjectTrackInfo(ObjectAccInfo* dstObjAccInfo, ObjectAccInfo* srcObjAccInfo, std::map<ObjectAccInfo*, ObjectAccInfo*>* addrMap) {
    for(unsigned int i = 0; i < srcObjAccInfo->trackSet.size(); i++) {
        ...
        for(unsigned int j = 0; j < srcObjAccInfo->trackSet[i]->size(); j++) {
            ...
            bool inTrack = false;
            for(unsigned int k = 0; k < dstObjAccInfo->trackSet[i]->size(); k++) {
                if((*addrMap)[srcObjAccInfo->trackSet[i]->at(j)] == dstObjAccInfo->trackSet[i]->at(k)) {
                    inTrack = true;
                    break;
                }
            }
            if(!inTrack) {
                dstObjAccInfo->trackSet[i]->push_back((*addrMap)[srcObjAccInfo->trackSet[i]->at(j)]);
            }
        }
...
void unionReturnObjs(MethodAccInfo* dstAccInfo, MethodAccInfo* srcAccInfo, std::map<ObjectAccInfo*, ObjectAccInfo*>* addrMap) {
    ...
    for(unsigned int i = 0; i < srcAccInfo->returnObjs->size(); i++) {
        bool inReturn = false;
        for(unsigned int j = 0; j < dstAccInfo->returnObjs->size(); j++) {
            if(dstAccInfo->returnObjs->at(j) == (*addrMap)[srcAccInfo->returnObjs->at(i)]) {
                inReturn = true;
                break;
            }
        }
        if(!inReturn) {
            dstAccInfo->returnObjs->push_back((*addrMap)[srcAccInfo->returnObjs->at(i)]);
        }
    }
}
...
void mergeObjectAccFieldInfo(std::vector<ObjectAccInfo*>* dstAccInfoList, ObjectAccInfo* srcAccInfo, std::map<ObjectAccInfo*, std::vector<ObjectAccInfo*> >* addrMap) {
    //ALOGE("dst acc list size: %u", dstAccInfoList->size());
    ...
    for(unsigned int i = 0; i < dstAccInfoList->size(); i++) {
        ...
        ALOGE("all the all srcAccInfo addr is: %p, dst addr is: %p", srcAccInfo, dstAccInfoList->at(i));
        if((unsigned int)dstAccInfoList->at(i) == 0x8301e10) {
            int level = 0;
            ObjectAccInfo* temp = srcAccInfo;
            do {
                level++;
                temp = temp->belonging;
                ALOGE("temp addr is: %p", temp);
            } while(temp != NULL);
            ALOGE("src addr is: %p, level is: %d", srcAccInfo, level);
            level = 0;
            temp = dstAccInfoList->at(i);
            do {
                level++;
                temp = temp->belonging;
            } while(temp != NULL);
            ALOGE("dst addr is: %p, level is: %d", dstAccInfoList->at(i), level);
        }
    }
    for(unsigned int i = 0; i < srcAccInfo->fieldSet.size(); i++) {
        ...
        for(unsigned int j = 0; j < dstAccInfoList->size(); j++) {
            if(dstAccInfoList->at(j)->trackSet[i] == NULL) { // indicates that the field accessed is the original field
                ...
                for(unsigned int k = 0; k < dstAccInfoList->at(j)->trackSet[i]->size(); k++) {
                    bool inField = false;
                    for(unsigned int l = 0; l < fieldList.size(); l++) {
                        if(fieldList.at(l) == dstAccInfoList->at(j)->trackSet[i]->at(k)) {
                            inField = true;
                            break;
                        }
                    }
                    if(!inField) {
                        fieldList.push_back(dstAccInfoList->at(j)->trackSet[i]->at(k));
                    }
                }
            }
        }
...
void mergeObjectAccTrackInfo(std::vector<ObjectAccInfo*>* dstAccInfoList, ObjectAccInfo* srcAccInfo, std::map<ObjectAccInfo*, std::vector<ObjectAccInfo*> >* addrMap);
...
void mergeObjectTrack(ObjectAccInfo* dstAccInfo, ObjectAccInfo* srcAccInfo, std::map<ObjectAccInfo*, std::vector<ObjectAccInfo*> >* addrMap) {
    ALOGE("single the all srcAccInfo addr is: %p, dst addr is: %p", srcAccInfo, dstAccInfo);
    for(unsigned int i = 0; i < srcAccInfo->trackSet.size(); i++) {
        
            if((unsigned int)dstAccInfo == 0x8301e10) {
            int level = 0;
            ObjectAccInfo* temp = srcAccInfo;
            do {
                level++;
            ...
            ALOGE("merge track src addr is: %p, level is: %d", srcAccInfo, level);
            level = 0;
            temp = dstAccInfo;
            do {
                level++;
            ...
            ALOGE("merge track dst addr is: %p, level is: %d", dstAccInfo, level);
                assert(dstAccInfo->trackSet[4] != NULL);
            }
        if(srcAccInfo->fieldSet[i] != NULL) {
            mergeObjectAccTrackInfo(dstAccInfo->trackSet[i], srcAccInfo->fieldSet[i], addrMap);
        }
        ...
        for(unsigned int j = 0; j < srcAccInfo->trackSet[i]->size(); j++) {
            ...
            for(unsigned int k = 0; k < (*addrMap)[srcAccInfo->trackSet[i]->at(j)].size(); k++) {
                bool inTrack = false;
                for(unsigned int l = 0; l < dstAccInfo->trackSet[i]->size(); l++) {
                    if((*addrMap)[srcAccInfo->trackSet[i]->at(j)].at(k) == dstAccInfo->trackSet[i]->at(l)) {
                        inTrack = true;
                        break;
                    }
                }
                if(!inTrack) {
                if((unsigned int)(*addrMap)[srcAccInfo->trackSet[i]->at(j)].at(k) == 0x8301e10) {
                    ALOGE("put the destination to an object, obj is: %p", dstAccInfo);
                }
                    dstAccInfo->trackSet[i]->push_back((*addrMap)[srcAccInfo->trackSet[i]->at(j)].at(k));
                }
            }
        }
    }
...
/*void mergeObjectTrack(ObjectAccInfo* dstAccInfo, ObjectAccInfo* srcAccInfo, std::map<ObjectAccInfo*, std::vector<ObjectAccInfo*> >* addrMap) {
    ALOGE("single the all srcAccInfo addr is: %p, dst addr is: %p", srcAccInfo, dstAccInfo);
    for(unsigned int i = 0; i < srcAccInfo->trackSet.size(); i++) {
        std::vector<ObjectAccInfo*>* oldTrack = new std::vector<ObjectAccInfo*>();
        if(srcAccInfo->fieldSet[i] != NULL) {
            oldTrack->insert(oldTrack->begin(), dstAccInfo->trackSet[i]->begin(), dstAccInfo->trackSet[i]->end());
        }
        if(srcAccInfo->trackSet[i] != NULL && srcAccInfo->trackSet[i]->size() != 0) {
            if(srcAccInfo->trackSet[i]->size() == 1 && srcAccInfo->trackSet[i]->at(0) == srcAccInfo->fieldSet[i]) {
                // indicates that the one in the track should not change, there is no assign to the track
        ...
        if(srcAccInfo->fieldSet[i] != NULL && oldTrack->size() != 0) {
            mergeObjectAccTrackInfo(oldTrack, srcAccInfo->fieldSet[i], addrMap);
        }
        free(oldTrack);
    }
}*/
...
void mergeObjectAccTrackInfo(std::vector<ObjectAccInfo*>* dstAccInfoList, ObjectAccInfo* srcAccInfo, std::map<ObjectAccInfo*, std::vector<ObjectAccInfo*> >* addrMap) {
    for(unsigned int i = 0; i < dstAccInfoList->size(); i++) {
        mergeObjectTrack(dstAccInfoList->at(i), srcAccInfo, addrMap);
    }
}
...
void mergeGlobalClazzField(MethodAccInfo* methodAccInfo, MethodAccInfo* subAccInfo, std::map<ObjectAccInfo*, std::vector<ObjectAccInfo*> >* addrMap) {
    for(unsigned int i = 0; i < subAccInfo->globalClazz->size(); i++) {
        ...
        //ALOGE("start global clazz merge");
        mergeObjectAccFieldInfo(&argVec, subAccInfo->globalClazz->at(i), addrMap);
    }
}

void mergeGlobalClazzTrack(MethodAccInfo* methodAccInfo, MethodAccInfo* subAccInfo, std::map<ObjectAccInfo*, std::vector<ObjectAccInfo*> >* addrMap) {
    for(unsigned int i = 0; i < subAccInfo->globalClazz->size(); i++) {
        unsigned int j;
        for(j = 0; j < methodAccInfo->globalClazz->size(); j++) {
            if(subAccInfo->globalClazz->at(i)->clazz == methodAccInfo->globalClazz->at(j)->clazz) {
        ...
        mergeObjectTrack(methodAccInfo->globalClazz->at(j), subAccInfo->globalClazz->at(i), addrMap);
        /*for(unsigned int k = 0; k < subAccInfo->globalClazz->at(i)->trackSet.size(); k++) {
            if(subAccInfo->globalClazz->at(i)->trackSet[k] != NULL && subAccInfo->globalClazz->at(i)->trackSet[k]->size() > 100) {
                ALOGE("sssub get an merged sub track at clazz, size is: %u", subAccInfo->globalClazz->at(i)->trackSet[k]->size());
            }
        }
        for(unsigned int k = 0; k < methodAccInfo->globalClazz->at(j)->trackSet.size(); k++) {
            if(methodAccInfo->globalClazz->at(j)->trackSet[k] != NULL && methodAccInfo->globalClazz->at(j)->trackSet[k]->size() > 100) {
                ALOGE("mmmet get an merged met track at clazz, size is: %u", methodAccInfo->globalClazz->at(j)->trackSet[k]->size());
            }
        }*/
    }
}
...
void mergeMethodReturnInfo(MethodAccInfo* methodAccInfo, MethodAccInfo* subAccInfo, std::map<ObjectAccInfo*, std::vector<ObjectAccInfo*> >* addrMap) {
    ...
    for(unsigned int i = 0; i < subAccInfo->returnObjs->size(); i++) {
        for(unsigned int j = 0; j < (*addrMap)[subAccInfo->returnObjs->at(i)].size(); j++) {
            bool inReturn = false;
            for(unsigned int k = 0; k < methodAccInfo->curMethodReturns->size(); k++) {
                if(methodAccInfo->curMethodReturns->at(k) == (*addrMap)[subAccInfo->returnObjs->at(i)].at(j)) {
                    inReturn = true;
                    break;
                }
            }
            if(!inReturn) {
                methodAccInfo->curMethodReturns->push_back((*addrMap)[subAccInfo->returnObjs->at(i)].at(j));
            }
        }
    }
}
...
void mergeMethodArgs(u2 vsrc1, u2 vdst, std::map<u2, std::vector<ObjectAccInfo*> >* interestRegObjMap, MethodAccInfo* methodAccInfo, MethodAccInfo* subAccInfo) {
    ALOGE("start a new method merge");
    u4 count = vsrc1 >> 4;
    u2 reg;
    ...
    case 5:
        reg = vsrc1 & 0x0f;
        if(interestRegObjMap->find(reg) != interestRegObjMap->end()) {
            mergeObjectAccTrackInfo(&((*interestRegObjMap)[reg]), subAccInfo->args->at(4), &addrMap);
        }
    case 4:
        reg = vdst >> 12;
        if(interestRegObjMap->find(reg) != interestRegObjMap->end()) {
            mergeObjectAccTrackInfo(&((*interestRegObjMap)[reg]), subAccInfo->args->at(3), &addrMap);
        }
    case 3:
        reg = (vdst & 0x0f00) >> 8;
        if(interestRegObjMap->find(reg) != interestRegObjMap->end()) {
            mergeObjectAccTrackInfo(&((*interestRegObjMap)[reg]), subAccInfo->args->at(2), &addrMap);
        }
    case 2:
        reg = (vdst & 0x00f0) >> 4;
        if(interestRegObjMap->find(reg) != interestRegObjMap->end()) {
            mergeObjectAccTrackInfo(&((*interestRegObjMap)[reg]), subAccInfo->args->at(1), &addrMap);
        }
    case 1:
        reg = vdst & 0x0f;
        if(interestRegObjMap->find(reg) != interestRegObjMap->end()) {
            mergeObjectAccTrackInfo(&((*interestRegObjMap)[reg]), subAccInfo->args->at(0), &addrMap);
        }
    default:
        ;
    }
    // merge global track info
    mergeGlobalClazzTrack(methodAccInfo, subAccInfo, &addrMap);
...
void mergeRangeMethodArgs(u2 vsrc1, u2 vdst, std::map<u2, std::vector<ObjectAccInfo*> >* interestRegObjMap, MethodAccInfo* methodAccInfo, MethodAccInfo* subAccInfo) {
    ...
    for(int i = 0; i < vsrc1; i++) {
        if(interestRegObjMap->find(vdst + i) != interestRegObjMap->end()) {
            mergeObjectAccTrackInfo(&((*interestRegObjMap)[vdst + i]), subAccInfo->args->at(i), &addrMap);
        }
    }
    // merge global track info
    mergeGlobalClazzTrack(methodAccInfo, subAccInfo, &addrMap);
        ...
        for (;;) {
            ...
            if(!isCycle) {
                ...
                if(newDepth == MaxBranchDepth) {
                    for(unsigned int i = 0; i < methodAccInfo->args->size(); i++) {
                        methodAccInfo->args->at(i)->allFlag = true;
                    }
                    *exitMethod = true;
                    return;
                }
...
void parseInsns(const u2* insns, MethodAccInfo* methodAccInfo, std::vector<Method*>* chain, std::vector<int>* insOffsets, std::map<u2, std::vector<ObjectAccInfo*> >* interestRegObjMap, int depth, bool* exitMethod) {
    ...
    for( ; ; lastOpcode = opcode, insns += width) {
        ...
        if(opcode == OP_IGET || opcode == OP_IGET_WIDE || opcode == OP_IGET_OBJECT || opcode == OP_IGET_BOOLEAN 
            ...
            if(interestRegObjMap->find(vdst) == interestRegObjMap->end() && interestRegObjMap->find(vsrc1) == interestRegObjMap->end()) {
                ...
                for(unsigned int j = 0; j < srcVector.size(); j++) {
                    ...
                    if(interestAccInfo->trackSet[offset] != NULL) {
                        free(interestAccInfo->trackSet[offset]);
                    }
            ...
            if(clazzAccInfo->trackSet[offset]->size() > 100) {
                ALOGE("SSSSput large size: %u", clazzAccInfo->trackSet[offset]->size());
            }
            interestRegObjMap->erase(vdst);
            // if the object is already set as migrate all, then ignore it
            ...
            if(interestRegObjMap->find(vdst) == interestRegObjMap->end()) {
                if(clazzAccInfo->trackSet[offset] != NULL) {
                    free(clazzAccInfo->trackSet[offset]);
                }
                clazzAccInfo->trackSet[offset] = new std::vector<ObjectAccInfo*>();
                continue;
            }
...
MethodAccInfo* parseMethod(Method* method, std::vector<Method*>* chain) {
    ...
    if(isCycle || dvmIsNativeMethod(method) || dvmIsAbstractMethod(method)) {
        for(unsigned int idx = 0; idx < methodAccInfo->args->size(); idx++) {
            methodAccInfo->args->at(idx)->allFlag = true;
        }
