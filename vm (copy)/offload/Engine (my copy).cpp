...
// list all the loaded classes
        ...
        dvmHashIterNext(&iter))
    {
        ...
        if (strncmp(clazz->descriptor, "Ledu/utk/offloadtest", 20) == 0) {
            ALOGE("loaded class: %s, classloader class: %s, classloader id: %d", clazz->descriptor, clazz->classLoader->clazz->descriptor, clazz->classLoader->objId);
        }
    }
...
ALOGI("Engine.cpp THREAD %d: SEND OWNERSHIP OF %d", self->threadId, objId);
    writeU4(fb, objId);
...
static void deactivate(Thread* self) {
  ALOGE("offsyncpush from deactivate in Engine.cpp");
  offSyncPushDoIf(NULL, NULL, (void(*)(void*, FifoBuffer*))doDeactivate, self);
}

void offMigrateThread(Thread* self) {
  ALOGE("Get into offMigrateThread, gDvm.offDisabled:%d, offWellConnected:%d, self->offProtection: %d, offRecoveryCheckEnterHazard:%d, gDvm.offConnected: %d", gDvm.offDisabled, offWellConnected(), self->offProtection, offRecoveryCheckEnterHazard(self), gDvm.offConnected);
  ...
  ALOGI("Migrating thread %d, current method %s", self->threadId, self->interpSave.method->name);
...
bool offPerformMigrate(Thread* self) {
  ...
  while(self->breakFrames > originalBreaks) {
    ...
    if(self->offFlagDeath) {
      ...
      //offWriteU1(self, OFF_ACTION_MIGRATE);
        ALOGE("deactivate from offPerformMigrate");
      //deactivate(self);
      return false;
    }
  }
...
void offIdSync(Thread* self) {
//    if(gDvm.offDisabled) return;
    ALOGE("get into offIdSync");
  //if(!gDvm.isServer && !offWellConnected()) return;
//  if(!gDvm.offConnected) return;
  ALOGE("get after check");
//  if(self->offProtection) return;
//  if(!offRecoveryCheckEnterHazard(self)) return;
...
//  self->migrationCounter++;
//  self->offDeactivateBreakFrames = self->breakFrames;
  offWriteU1(self, OFF_ACTION_ID_SYNC);
  offWriteU4(self, 12345);
  //deactivate(self);
}

void offPerformIdSync(Thread* self) {
    ALOGE("perform id sync");
    u4 val = offReadU4(self);
    ALOGE("perform id sync value: %d", val);
}
...
void offMigrateClinit(Thread* self, ClassObject* clazz) {
  ...
  ALOGE("deactivate from offMigrateClinit");
  deactivate(self);
  offThreadWaitForResume(self);
  if(!activate(self)) {
...
void offPerformClinit(Thread* self) {
  ...
  ALOGE("deactivate from offPerformClinit");
  deactivate(self);
}
