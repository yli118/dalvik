...
void offMigrateThread(Thread* self) {
  ...
  while(fp != NULL) {
    ...
    if(method == NULL) {
        ALOGE("before perform migrate get a null");
    } else {
        ALOGE("before perform migrate: class:%s, method: %s", method->clazz->descriptor, method->name);
    }
    fp = saveArea->prevFrame;
  }
  ALOGE("count of breaks when migrate: %d", self->breakFrames);
...
bool offPerformMigrate(Thread* self) {
  ...
  if(!activate(self)) {
    ALOGE("perform migrate activate fail!");
    return false;
  } else {
    ALOGE("perform migrate activate succeed!");
  }
  sst = &self->interpSave;
  fp = sst->curFrame;
  while(fp != NULL) {
    ...
    Method* method = (Method*)saveArea->method;
    if(method == NULL) {
        ALOGE("after perform migrate get a null");
    } else {
        ALOGE("after perform migrate: class:%s, method: %s", method->clazz->descriptor, method->name);
    }
    fp = saveArea->prevFrame;
  }
  ALOGE("perform migrate breaks: %d, originalBreaks: %d", self->breakFrames, originalBreaks);
  while(self->breakFrames > originalBreaks) {
    ALOGE("get into perform migrate while loop");
    ...
    if(self->offFlagDeath) {
      return true;
        ...
        ALOGE("perform migrate back to local from breaks");
