        ...
        bool ignoreUnrecognized, JNIEnv* pEnv)
{
        ...
        dvmHashIterNext(&iter))
    {
        ...
        ALOGE("loaded class: %s, id: %d", clazz->descriptor, clazz->objId);
    }
    // list all the dvms
    for (u4 i = 0; i < auxVectorSize(&gDvm.dexList); i++) {
        DvmDex* pDvmDex = (DvmDex*)auxVectorGet(&gDvm.dexList, i).v;
        ALOGE("dvm dex info: %s, id: %d",  pDvmDex->cacheFile, pDvmDex->id);
    }
...
bool dvmInitAfterZygote()
{
    ...
    if (!offControlStartup(true))
        return false;
    //if (!gDvm.isServer) {
    //while(!gDvm.isServer && !offWellConnected()) {
    //    sleep(5);
    //}
    //Thread* self = dvmThreadSelf();
    //offIdSync(self);
        /*ALOGE("Send id sync message from thread: %d", self->threadId);
